...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, modelID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("model_%d%s", modelID, ext)
...
func UploadVideo(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, modelID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("model_%d_video%s", modelID, ext)
